...
function InlineDatePicker({ value, onChange }: InlineDatePickerProps) {
  ...
  const selectDay = (day: number) => {
    ...
    // Explicit format matching local timezone alignment
  ...
  return (
    <div className="rounded-3xl border border-slate-200/80 bg-white/95 p-4 space-y-3 shadow-sm">
        ...
        <div className="flex gap-2 flex-1 justify-center">
          <div className="relative">
            <select
              ...
              className="appearance-none bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-xs font-semibold px-3 pr-7 py-1.5 cursor-pointer focus:outline-none transition-all hover:border-slate-300"
            >
              {MONTHS.map((m, i) => (
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-xs font-semibold px-3 pr-7 py-1.5 cursor-pointer focus:outline-none transition-all hover:border-slate-300 w-[72px]"
            >
              {years.map(y => (
          ...
          return (
            <button
              ...
              className={cn(
                ...
                sel && "bg-blue-600 text-white font-bold shadow-lg shadow-blue-900/20 ring-2 ring-blue-500/30",
              )}
            >
              {cell.day}
              {todayMark && !sel && (
      ...
      <div className={cn("flex items-center gap-3 rounded-xl px-3.5 py-2.5 border transition-all duration-300", selected ? "bg-blue-600/10 border-blue-500/30" : "bg-slate-50 border-slate-200")}>
        <CalendarIcon className={cn("w-4 h-4 shrink-0", selected ? "text-blue-400" : "text-slate-500")} />
        <div className="flex-1 min-w-0">
          {selected ? (
            <p className="text-[13px] font-semibold text-slate-900 truncate">{formatDisplay(selected)}</p>
          ) : (
        ...
        {selected && (() => {
          ...
          return age !== null ? (
            <span className="shrink-0 text-[11px] font-bold text-blue-400 bg-blue-500/15 border border-blue-500/20 rounded-lg px-2 py-0.5">{age} yrs</span>
          ) : null;
        })()}
      </div>
    </div>
...
const StepGoal = ({ formData, setFormData, handleNext }: StepProps) => (
  <div className="space-y-6">
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        <button
          ...
          className={cn("flex items-center gap-4 p-5 rounded-2xl border transition-all duration-300 text-left group", formData.userGoal === "OFFER_SERVICE" ? "bg-blue-500/10 border-blue-500/50 ring-1 ring-blue-500/50" : "bg-slate-50 border-slate-200 hover:bg-slate-100 hover:border-slate-300")}
        >
          <div className={cn("p-3 rounded-xl transition-colors", formData.userGoal === "OFFER_SERVICE" ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-600 group-hover:text-slate-900")}>
            <Handshake className="w-5 h-5" />
          </div>
          <div>
            <p className={cn("text-sm font-bold", formData.userGoal === "OFFER_SERVICE" ? "text-slate-900" : "text-slate-700")}>Offer my services</p>
        ...
        <button
          ...
          className={cn("flex items-center gap-4 p-5 rounded-2xl border transition-all duration-300 text-left group", formData.userGoal === "HIRE_PROFESSIONALS" ? "bg-purple-500/10 border-purple-500/50 ring-1 ring-purple-500/50" : "bg-slate-50 border-slate-200 hover:bg-slate-100 hover:border-slate-300")}
        >
          <div className={cn("p-3 rounded-xl transition-colors", formData.userGoal === "HIRE_PROFESSIONALS" ? "bg-purple-500 text-white" : "bg-slate-100 text-slate-600 group-hover:text-slate-900")}>
            <Target className="w-5 h-5" />
          </div>
          <div>
            <p className={cn("text-sm font-bold", formData.userGoal === "HIRE_PROFESSIONALS" ? "text-slate-900" : "text-slate-700")}>Hire professionals</p>
    ...
    <Button onClick={handleNext} disabled={!formData.userGoal} className="w-full h-11 bg-blue-600 text-white hover:bg-blue-700 font-bold mt-4 rounded-xl">
      Continue <ArrowRight className="ml-2 w-4 h-4" />
    </Button>
  </div>
);

const StepPersonalInfo = ({ formData, setFormData, handleNext, handleBack }: StepProps) => {
  // Enforces matching Indian telephone formats (Strict 10 digits starting with 6-9)
  const isIndianPhoneValid = /^[6-9]\d{9}$/.test(formData.phoneNumber);

  const handlePhoneInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    const rawVal = event.target.value.replace(/\D/g, ""); // Instantly strip non-numeric entries
    if (rawVal.length <= 10) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4">
        <div className="space-y-1.5">
          ...
          <Input value={formData.displayName} onChange={e => setFormData({ ...formData, displayName: e.target.value })} className="bg-slate-50 border-slate-200 text-slate-900 h-11 rounded-xl" />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative flex items-center">
            <span className="absolute left-3.5 text-xs font-semibold text-slate-400 select-none pointer-events-none">+91</span>
            <Input 
              type="text"
              inputMode="numeric"
              value={formData.phoneNumber} 
              onChange={handlePhoneInput} 
              className="bg-slate-50 border-slate-200 text-slate-900 h-11 rounded-xl pl-12" 
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <select
            id="gender"
            className="w-full bg-slate-50 border border-slate-200 rounded-xl outline-none px-3 py-2.5 text-sm text-slate-900 appearance-none focus:ring-2 focus:ring-blue-500/50"
      ...
      <div className="flex gap-3 pt-2">
        <Button variant="ghost" onClick={handleBack} className="flex-1 h-11 text-slate-600 hover:bg-slate-100 rounded-xl">
          <ArrowLeft className="mr-2 w-4 h-4" /> Back
        </Button>
        <Button
          onClick={handleNext}
          disabled={!formData.displayName || !isIndianPhoneValid || !formData.gender || !formData.dateOfBirth || calculateAge(formData.dateOfBirth) === null}
          className="flex-[2] h-11 bg-blue-600 text-white hover:bg-blue-700 font-bold rounded-xl"
        >
          Continue <ArrowRight className="ml-2 w-4 h-4" />
        </Button>
      </div>
...
const StepProfession = ({ formData, setFormData, handleNext, handleBack }: StepProps) => {
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          {CATEGORIES.map(cat => (
            <button
              ...
              className={cn("flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 text-left", formData.category === cat.id ? `${cat.bg} ${cat.border} ring-2 ring-blue-500/20` : "bg-slate-50 border-slate-200 hover:bg-slate-100 hover:border-slate-300")}
            >
              <div className={cn("p-2 rounded-lg", cat.bg, cat.color)}>
                <cat.icon className="w-4 h-4" />
              </div>
              <span className={cn("text-xs font-semibold", formData.category === cat.id ? "text-slate-900" : "text-slate-700")}>{cat.label}</span>
            </button>
          ))}
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {SUB_PROFESSIONS[formData.category].map(prof => (
              <button
                ...
                className={cn("px-3 py-1.5 rounded-full text-[11px] font-medium transition-all border", formData.profession === prof ? "bg-blue-600 border-blue-500 text-white shadow-md shadow-blue-900/10" : "bg-slate-50 border-slate-200 text-slate-700 hover:border-slate-300")}
              >
                {prof}
              </button>
            ))}
            <button
              ...
              className={cn("px-3 py-1.5 rounded-full text-[11px] font-medium transition-all border", formData.profession === "Other" ? "bg-blue-600 border-blue-500 text-white" : "bg-slate-50 border-slate-200 text-slate-700 hover:border-slate-300")}
            >
              Other...
            </button>
          </div>
      ...
      {(formData.category === "other" || formData.profession === "Other") && (
        <div className="space-y-2">
          ...
          <Input value={formData.customProfession} onChange={e => setFormData({ ...formData, customProfession: e.target.value })} className="bg-slate-50 border-slate-200 text-slate-900 h-11 rounded-xl" placeholder="Type your skill..." />
        </div>
      )}
...
const StepBio = ({ formData, setFormData, handleBack, handleSubmit, isSubmitting }: StepProps) => (
  <div className="space-y-4">
    <div className="space-y-1.5">
      <label className="text-[10px] font-bold tracking-widest uppercase text-slate-500 ml-1">Bio (Optional)</label>
      <Textarea value={formData.bio} onChange={e => setFormData({ ...formData, bio: e.target.value })} className="bg-slate-50 border-slate-200 text-slate-900 min-h-[120px] resize-none placeholder:text-slate-400 rounded-xl p-3 text-sm leading-relaxed" placeholder="I'm a passionate developer focusing on..." />
      <p className="text-[10px] text-slate-500 mt-1">Tell people what makes you special.</p>
    </div>

    <div className="flex gap-3 mt-6">
      <Button variant="ghost" onClick={handleBack} className="flex-1 h-11 text-slate-600 hover:bg-slate-100 rounded-xl" disabled={isSubmitting}>
        <ArrowLeft className="mr-2 w-4 h-4" /> Back
      </Button>
      <Button onClick={handleSubmit} className="flex-[2] h-11 bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all shadow-md rounded-xl" disabled={isSubmitting}>
        {isSubmitting ? (
...
export function ProfileCompletionModal() {
  ...
  const stepMeta = [
    { icon: <Handshake className="w-5 h-5" />,    title: "Start Your Journey",   desc: "What brings you to Krovaa today?" },
    { icon: <UserIcon className="w-5 h-5" />,     title: "Personal Presence",    desc: "Help us get to know you better." },
    { icon: <Briefcase className="w-5 h-5" />,    title: "Identity Alignment",   desc: "Tell us about your skills and expertise." },
    { icon: <CheckCircle2 className="w-5 h-5" />, title: "Final Details",        desc: "A small blueprint overview goes a long way." },
  ];
  ...
  return (
    <Dialog open={true} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[480px] w-[calc(100%-32px)] overflow-hidden bg-white/95 backdrop-blur-xl border border-slate-200 p-0 shadow-2xl rounded-3xl z-[100]" hideCloseButton>
        ...
        <div className="p-6 sm:p-8">
          <div className="mb-6 text-center">
            <div className="inline-flex items-center justify-center w-11 h-11 rounded-2xl bg-blue-50 border border-blue-100 text-blue-600 mb-3 shadow-sm">
              {meta.icon}
            </div>
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-slate-900 tracking-tight text-center">{meta.title}</DialogTitle>
              <DialogDescription className="text-slate-500 text-xs mt-0.5 text-center">{meta.desc}</DialogDescription>
            </DialogHeader>
          </div>

          <div className="mt-2 min-h-[340px]">
            {step === 1 && <StepGoal formData={formData} setFormData={setFormData} handleNext={handleNext} />}
            {step === 2 && <StepPersonalInfo formData={formData} setFormData={setFormData} handleNext={handleNext} handleBack={handleBack} />}
            {step === 3 && <StepProfession formData={formData} setFormData={setFormData} handleNext={handleNext} handleBack={handleBack} />}
            {step === 4 && <StepBio formData={formData} setFormData={setFormData} handleBack={handleBack} handleSubmit={handleSubmit} isSubmitting={isSubmitting} />}
          </div>

          <div className="mt-6 flex justify-center gap-1.5">
            {[1, 2, 3, 4].map(s => (
              <div key={s} className={cn("h-1 rounded-full transition-all duration-300", step === s ? "w-5 bg-blue-600" : "w-1 bg-slate-200")} />
            ))}
          </div>
        </div>
      </DialogContent>
