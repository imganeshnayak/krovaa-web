...
async function run() {
  try {
    ...
    console.log('Fetching /api/admin/chats? via Vite');
    const res1 = await fetch('http://localhost:5173/api/admin/chats?', {
    ...
    console.log('\nFetching /api/admin/group-chats? via Vite');
    const res2 = await fetch('http://localhost:5173/api/admin/group-chats?', {
      headers: { Authorization: `Bearer ${token}` }
    });
